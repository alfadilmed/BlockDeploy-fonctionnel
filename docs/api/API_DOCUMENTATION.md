...
## ERC-721 Contract Actions

The following endpoints allow authenticated users to perform actions on ERC-721 contracts they own or manage.

### Pause ERC-721 Contract

*   **Endpoint:** `POST /api/v1/contracts/:network/:address/erc721/pause`
*   **Description:** Attempts to pause a pausable ERC-721 smart contract. This typically restricts token transfers and minting.
*   **Authentication:** Requires Bearer Token. User must have appropriate permissions (e.g., owner or pauser role) on the contract.
*   **Path Parameters:**
    *   `network` (string, required): The network of the contract (e.g., "sepolia").
    *   `address` (string, required): The contract address.
*   **Request Body:** None.
*   **Success Response (200 OK):**
    ...
    *   **Example:**
        ...
        {
          "success": true,
          "transactionHash": "0xerc721pausehash...",
          "message": "ERC721 Pause action initiated successfully for contract 0x... Transaction hash: 0xerc721pausehash..."
        }
        ```
*   **Error Responses:** (Similar to ERC-20 Pause)
    *   `400 Bad Request`: If the contract is already paused, or not pausable.
    *   `401 Unauthorized`, `403 Forbidden`, `404 Not Found`, `500 Internal Server Error`.

---

### Unpause ERC-721 Contract

*   **Endpoint:** `POST /api/v1/contracts/:network/:address/erc721/unpause`
*   **Description:** Attempts to unpause a pausable ERC-721 smart contract that is currently paused.
*   **Authentication:** Requires Bearer Token. User must have appropriate permissions.
*   **Path Parameters:**
    *   `network` (string, required): The network of the contract.
    *   `address` (string, required): The contract address.
...
*   **Success Response (200 OK):**
    ...
    *   **Example:**
        ...
        {
          "success": true,
          "transactionHash": "0xerc721unpausehash...",
          "message": "ERC721 Unpause action initiated successfully for contract 0x... Transaction hash: 0xerc721unpausehash..."
        }
        ```
*   **Error Responses:** (Similar to ERC-20 Unpause)
    *   `400 Bad Request`: If the contract is not currently paused, or not pausable.
    *   `401 Unauthorized`, `403 Forbidden`, `404 Not Found`, `500 Internal Server Error`.

---

### Mint NFT (ERC-721 Token)

*   **Endpoint:** `POST /api/v1/contracts/:network/:address/erc721/mint`
*   **Description:** Mints a new NFT for a compatible ERC-721 smart contract.
*   **Authentication:** Requires Bearer Token. User must have appropriate permissions (e.g., owner or minter role).
*   **Path Parameters:**
    *   `network` (string, required): The network of the contract.
    *   `address` (string, required): The contract address.
*   **Request Body (`NftMintRequestDto`):**
    *   `recipient` (string, required, Ethereum address): The address to receive the new NFT.
    *   `tokenId` (string, required, positive integer string): The unique ID for the new NFT.
    *   `tokenURI` (string, required): The URI for the NFT's metadata. For contracts like \`ERC721MVP.sol\` that generate \`tokenURI\` from a \`baseURI\` and \`tokenId\`, this provided \`tokenURI\` might be logged by the backend but not directly passed to the contract's standard mint function if it doesn't accept it. The backend will call \`safeMintWithId(recipient, tokenId)\`. The actual \`tokenURI\` will be generated by the contract. (See \`L4_M7_NFT_IPFS_UPLOAD_OPTION2.md\` for creating metadata and obtaining a \`tokenURI\` if your contract expects unique URIs per token and has a mint function that accepts it).
    *   **Example:**
        ```json
        {
          "recipient": "0xNftRecipientAddress...",
          "tokenId": "12345",
          "tokenURI": "ipfs://QmWGA3NAGSG1dYb8V7c3bKFAbK3sNXT9YQnk2FepD2b7Ld/12345.json"
        }
        ```
*   **Success Response (200 OK):**
    *   **Content-Type:** `application/json`
    ...
    *   **Example:**
        ...
        {
          "success": true,
          "transactionHash": "0xerc721minthash...",
          "message": "NFT Mint action initiated successfully for contract 0x... to 0xNftRecipientAddress.... TokenID: 12345. Transaction hash: 0xerc721minthash..."
        }
        ```
*   **Error Responses:**
