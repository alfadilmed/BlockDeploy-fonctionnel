...
const StepProgress = ({ steps = ["Select Type", "Configure", "Deploy"], currentStep = 1, className = '' }) => {
    ...
    <nav aria-label="Progress" className={`w-full ${className}`}>
      <ol role="list" className="flex items-start space-x-1 sm:space-x-2">
        {steps.map((stepName, index) => {
          ...
          let statusStyles = {
            iconContainer: 'bg-slate-100 dark:bg-slate-700 border-slate-300 dark:border-slate-600',
          ...
          if (isCompleted) {
            ...
            statusStyles = {
              iconContainer: 'border-brand-accent-blue ring-1 sm:ring-2 ring-brand-accent-blue bg-slate-100 dark:bg-slate-700',
              icon: 'text-brand-accent-blue',
              name: 'text-brand-accent-blue font-semibold',
              connector: 'bg-slate-300 dark:bg-slate-700', // Connector leading to current is upcoming style
            };
          }
          ...
          return (
            <li key={stepName} className={`relative ${index < steps.length - 1 ? 'flex-1' : ''}`}>
              ...
              {index < steps.length - 1 && (
                <div 
                    className="absolute left-1/2 top-[calc(0.875rem)] -translate-x-1/2 sm:left-[calc(1rem_-_1px)] sm:top-[calc(1rem_-_1px)] h-0.5 w-full" 
                    aria-hidden="true"
                    style={{ zIndex: -1 }}
                >
                   <div className={`absolute top-0 h-0.5 ${statusStyles.connector} transition-colors duration-300`} style={{left: 'calc(50% + 0.5rem)', width: 'calc(100% - 1rem)'}}/>
                </div>
              )}
            </li>
          );
